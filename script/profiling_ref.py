...
if not os.path.exists(f'{data_dir}profiling_ref_times.csv'):
    for n in n_obj:
        ...
        mem_alloc_time.append(record[0])
    ...
    with open(f'{data_dir}profiling_ref_times.csv', 'w') as f:
        ...
        for i in range(len(n_obj_accurate)):
            f.write(f"{n_obj_accurate[i]},{mem_alloc_time[i]},{create_world_time[i]},{render_time[i]},{save_image_time[i]},{free_cuda_time[i]}\n")
else:
    with open(f'{data_dir}profiling_ref_times.csv', 'r') as f:
        ...
        for line in lines:
            ...
            mem_alloc_time.append(float(parts[1]))
...
x = range(len(labels))
plt.bar(x, mem_alloc_time, label='Memory allocation')
plt.bar(x, create_world_time, bottom=mem_alloc_time, label='World creation')
bottoms = [i+j for i,j in zip(mem_alloc_time, create_world_time)]
