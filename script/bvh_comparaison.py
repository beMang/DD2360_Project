...
ref_exec_2 = "./bin/cudart_ref2" #this is not the right reference for the wanted plot.
...
if not os.path.exists(f'{data_dir}bvh_comparison_times.csv'):
    for n in n_obj:
        ...
        for _ in range(n_run):
            ...
            #lanch subprocess to run the second reference executable
            ref_result_2 = subprocess.run([ref_exec_2, str(n)], stdout=subprocess.PIPE, text=True)
            ref2_time += get_time_from_output(ref_result_2.stdout)
        print(f'Completed for n_obj={n}')

        times.append(opti_time / n_run)
        ref_times.append(ref_time / n_run)
        ref_times_2.append(ref2_time / n_run)
    #save data to file
    print(n_obj_accurate)
    np.savetxt(f'{data_dir}bvh_comparison_times.csv', np.array([n_obj, n_obj_accurate, times, ref_times, ref_times_2]).T, header='n_obj,n_obj_accurate,opti_time,ref_time,ref_time_2', delimiter=',')
else:
    ...
    ref_times_2 = data[:,4].tolist()

plt.plot(n_obj_accurate, ref_times, '--x', label='Reference', color='blue', alpha=0.8)
plt.plot(n_obj_accurate, ref_times_2, '--x', label='BHV only', color='red', alpha=0.8)
plt.plot(n_obj_accurate, times, '--x', label='BHV and // GPU scene', color='green', alpha=0.8)
...
plt.tight_layout()
plt.savefig(f'{img_dir}bvh_comparison.pdf')
plt.close()
